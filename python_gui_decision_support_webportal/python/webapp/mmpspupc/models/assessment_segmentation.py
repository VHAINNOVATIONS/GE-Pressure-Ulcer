...
class AssessmentSegmentation(Base):
    ...
    keyCol = 'id'
    editCols = ['start_time','image_label_map_file', 'granulation_percentage', 'slough_percentage', 'eschar_percentage', 'bone_percentage']
    editColsLabels = ['Start Time','image_label_map_file', 'Granulation Percentage', 'Slough Percentage', 'Eschar Percentage', 'Bone Percentage']
    editColsTypes = ['date','string','numeric','numeric','numeric','numeric']
    ...
    def __json__(self, request):
        return {'id':self.id, 'session_id':self.session_id, 'experiment_id':self.experiment_id, 'start_time':self.start_time.isoformat(' '),
                'image_label_map_file':self.image_label_map_file, 'granulation_percentage':self.granulation_percentage, 'slough_percentage':self.slough_percentage, 'eschar_percentage':self.eschar_percentage, 'bone_percentage':self.bone_percentage }

    def __repr__(self):
        return "<AssessmentSegmentation(id='%d', session_id='%d', experiment_id='%d', start_time='%s', image_label_map_file='%d', granulation_percentage='%d', slough_percentage='%d', eschar_percentage='%d', bone_percentage='%d')>" % (
                self.id, self.session_id, self.experiment_id, self.start_time, self.image_label_map_file, self.granulation_percentage, self.slough_percentage, self.eschar_percentage, self.bone_percentage)
