...
from wound_assessment_olv import OlvWoundAssessment, OlvWoundAssessmentCols
...
########################################################################
class WoundAssessmentDialog(wx.Dialog):
    """
    ...
    #----------------------------------------------------------------------
    def __init__(self, parent, db, patientId, mode="Add-Update-Delete"):
        """
        ...
        self.db = db
        self.obj = WoundAssessment
        self.objOlv = OlvWoundAssessment
        self.objOlvCols = OlvWoundAssessmentCols()
        width = self.objOlvCols.getTotalColumnWidth()
        wx.Dialog.__init__(self, parent, size=wx.Size(width,500))
        self.controller = WoundAssessmentController(db, self.obj, self.objOlv, self.objOlvCols)
        try:
    ...
    def onAddRecord(self, event):
        ...
        if selectedRow == None:
            ...
            return
        dlg = AddModifyWoundDialog(self.controller, self.obj, self.db, row=selectedRow, title="Add",
                                           addRecord=True)
        rc = dlg.ShowModal()
        if rc == 0:
            self.showAllRecords()
    ...
    def onEditRecord(self, event):
        ...
        if selectedRow == None:
            ...
            return
        dlg = AddModifyWoundDialog(self.controller, self.obj, self.db, row=selectedRow, title="Modify",
                                           addRecord=False)
        rc = dlg.ShowModal()
        if rc == 0:
            self.showAllRecords()
