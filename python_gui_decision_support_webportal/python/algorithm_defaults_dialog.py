...
from add_modify_algorithm_defaults_dialog import AddModifyAlgorithmDefaultsDialog

########################################################################
class AlgorithmDefaultsDialog(wx.Dialog):
    """
    ...
    #----------------------------------------------------------------------
    def __init__(self, parent, db, algorithmId, mode="Add-Update-Delete"):
        """
        ...
        self.db = db
        self.obj = AlgorithmDefaults
        self.objOlv = OlvAlgorithmDefaults
        self.objOlvCols = OlvAlgorithmDefaultsCols()
        width = self.objOlvCols.getTotalColumnWidth()
        wx.Dialog.__init__(self, parent, size=wx.Size(width,500))
        self.controller = AlgorithmDefaultsController(db, self.obj, self.objOlv, self.objOlvCols)
        try:
    ...
    def onAddRecord(self, event):
        ...
        """
        dlg = AddModifyAlgorithmDefaultsDialog(self.controller, self.obj, self.db, title="Add",
                                           addRecord=True, algorithmId=self.algorithm_id)
        rc = dlg.ShowModal()
        if rc == 0:
            self.showAllRecords()
    ...
    def onEditRecord(self, event):
        ...
        if selectedRow == None:
            ...
            return
        dlg = AddModifyAlgorithmDefaultsDialog(self.controller, self.obj, self.db, row=selectedRow, title="Modify",
                                           addRecord=False)
        rc = dlg.ShowModal()
        if rc == 0:
            self.showAllRecords()
