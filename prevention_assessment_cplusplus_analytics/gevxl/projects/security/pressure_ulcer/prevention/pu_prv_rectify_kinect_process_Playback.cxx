...
void pu_prv_rectify_kinect_process::perform_depth_filtering(void)
{
  vil_image_view<vxl_byte>::iterator out_depth_filtered_itr = depth_filtered_img_.begin();
	...
	vil_image_view<vxl_uint_16>::const_iterator depth_itr  = depth_img_.begin();
  ...
  while(depth_itr != depth_img_.end()) {
...
bool pu_prv_rectify_kinect_process::step( const vil_image_view<vxl_uint_16> depth_img, const vil_image_view<float> &xyzrgb_img )
{
  depth_img_.deep_copy(depth_img);
