...
#include <pressure_ulcer/prevention/pu_prevention_pose_estimate_chaining_process.h>
...
public:
  ...
  void set_proc(pu_prevention_pose_estimate_chaining_process *p) { proc_ = p; }

  void on_configure(wxCommandEvent &evt);

private:

  pu_prevention_pose_estimate_chaining_process *proc_;
...
public:
  ...
  void set_process(pu_prevention_pose_estimate_chaining_process *p) { proc_ = p; } 
...
demo_frame::~demo_frame(void) 
{
  ...
  if(NULL != options_handler_) {
    // -- Deleting this seems to crash the system upon exit
