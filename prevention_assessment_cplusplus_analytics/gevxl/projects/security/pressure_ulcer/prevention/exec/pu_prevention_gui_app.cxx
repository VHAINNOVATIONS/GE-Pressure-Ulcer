...
public:
  ...
  void set_proc(pu_prevention_chaining_process *p) { proc_ = p; }

  void on_configure(wxCommandEvent &evt);

private:

  pu_prevention_chaining_process *proc_;
...
public:
  ...
  demo_frame(const vcl_string &title)
    : frame(NULL, title, wxPoint(0,0) /* wxDefaultPosition*/, wxSize(1480,480)), 
    canvas_(NULL),
    proc_(NULL),
    record_client_(NULL),
  ...
  void set_process(pu_prevention_chaining_process *p) { proc_ = p; } 
...
protected:
  ...
  options_handler_client *options_handler_;
	...
	// the pose estimate client
	gevxl::pressure_ulcer::prevention::pu_pose_estimate_client *pose_estimate_client_;
...
demo_frame::~demo_frame(void) 
{
	...
	canvas_->remove_client(options_handler_);
	...
	if(NULL != options_handler_) {
		RemoveEventHandler(options_handler_);
...
bool demo_frame::initialize(void)
{
  ...
  // Allocate canvas
  canvas_ = new wx::canvas(get_panel(), wxid("Canvas"), wxPoint(0,0) /*wxDefaultPosition*/, 
    wxSize(2*proc_->cur_frame().ni()+200, proc_->cur_frame().nj()), 0, ("PUDepthCanvas"), gl_attrib );
  ...
  options_handler_->set_proc(proc_);
	...
	canvas_->add_client(options_handler_);
  
	pose_estimate_client_ = new gevxl::pressure_ulcer::prevention::pu_pose_estimate_client();
	pose_estimate_client_->set_prevention_chaining_proc(proc_);
...
bool MyApp::OnInit()
{  
  vul_arg<vcl_string> a_config_file( "-c", "Config file", "C:/GEVXL/gevxl/projects/security/pressure_ulcer/prevention/pu_prevention.tconf" );
  ...
  frame_ = demo_frame_sptr(new demo_frame("Pressure Ulcer Prevention GUI Demo"));
