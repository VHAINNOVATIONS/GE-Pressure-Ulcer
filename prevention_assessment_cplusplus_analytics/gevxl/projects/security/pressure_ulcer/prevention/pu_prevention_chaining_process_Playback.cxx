...
bool pu_prevention_chaining_process::configure(util::config_file &config)
{
  ...
  running_mode_ = "live";
  config.get_string(name()+"::running_mode", running_mode_);
  ...
  // gevxl::pressure_ulcer::prevention::pu_prevention_videoarchive_frame_process
  // configure videoarchive_frame_proc_
  if( !videoarchive_frame_proc_.configure(config) ) {
    vcl_cerr << "pu_prevention_chaining_process::configure, Error configuring videoarchive_frame_proc_." << vcl_endl;
		return false;
  }
...
bool pu_prevention_chaining_process::initialize(void)
{
  ...
  else if(running_mode_ == "playback") {
    if( !videoarchive_frame_proc_.initialize() ) {
		  vcl_cerr << "pu_prevention_chaining_process::initialize, Error initializing videoarchive_frame_proc_." << vcl_endl;
		  return false;
	  }

	  frame_tag_proc_.set_source_tagged_frame_process(&videoarchive_frame_proc_);
	  if(!frame_tag_proc_.initialize()) {
		  vcl_cerr << "pu_prevention_chaining_process::initialize, Error initializing frame_tag_proc_." << vcl_endl;        
		  return false;
	  }
...
void pu_prevention_chaining_process::uninitialize(void)
{
  ...
  videoarchive_frame_proc_.uninitialize();
}
...
bool pu_prevention_chaining_process::step(void)
{
  ...
  // Stepping through individual process in the chain
    ...
    if( !videoarchive_frame_proc_.step() ) {
		  vcl_cerr << "pu_prevention_chaining_process::step, stepping videoarchive_frame_proc_ error." << vcl_endl;
		  return false;
	  }
  }
	...
	// --- Rectify
  vil_image_view<vxl_byte> rgb_img, depth_img;
  ...
  if(running_mode_ == "live") {
    ...
    rgb_img = openni2_proc->cur_rgb_frame();
	  depth_img = openni2_proc->cur_depth_byte_frame();

    // rectification process
    if( !rectify_kinect_proc_.step( openni2_proc->cur_depth_frame(), openni2_proc->cur_xyz_rgb_frame() ) ) {
		  vcl_cerr << "pu_prevention_chaining_process::step(), Error executing rectify_kinect_proc_.step()." << vcl_endl;
		  return false;
	  }
  }
  else if(running_mode_ == "playback") {
    rgb_img = videoarchive_frame_proc_.cur_rgb_frame();
	  depth_img = videoarchive_frame_proc_.cur_depth_byte_frame();

    // rectification process
    if( !rectify_kinect_proc_.step( videoarchive_frame_proc_.cur_depth_frame(), videoarchive_frame_proc_.cur_xyz_rgb_frame() ) ) {
		  vcl_cerr << "pu_prevention_chaining_process::step(), Error executing rectify_kinect_proc_.step()." << vcl_endl;
		  return false;
	  }
  }
...
const vil_image_view<vxl_byte> &pu_prevention_chaining_process::cur_frame(void) const
{
  if(running_mode_ == "live") {
    ...
    if(cur_frame_output_type_ == "depth") {
		  videoarchive_frame_proc_.cur_depth_byte_frame();
    }
	  else if(cur_frame_output_type_ == "rgb") {
		  videoarchive_frame_proc_.cur_rgb_frame();
	  }
    else if(cur_frame_output_type_ == "motion") {
      return motion_estimate_proc_.get_visualization_image();
    }
